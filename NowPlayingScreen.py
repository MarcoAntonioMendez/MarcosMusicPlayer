...
class NowPlayingScreen:
	...
	def __init__(self, presentation_screen: PresentationScreen, console: Console, textManager: TextManager, inputListener: InputListener, tabsManager: TabsManager):
		...
		self.current_y_pos = self.NOW_PLAYING_TAB_INDEX
	...
	def update(self):
		...
		self.on_key_pressed(key = self.inputListener.listen_for_key_press())
	...
	def process_enter_input(self):
		if( self.tabsManager.is_exit_tab_selected(x_pos = self.current_x_pos, y_pos = self.current_y_pos) ):
			self.return_action = self.presentation_screen.EXIT_SCREEN_ID
		elif( self.tabsManager.is_settings_tab_selected(x_pos = self.current_x_pos, y_pos = self.current_y_pos) ):
